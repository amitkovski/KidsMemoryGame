        ...
        if (cardSelection.length == 2) {
          ...
          if (this.cards[cardSelection[0]].value !=
            ...
            if (revealedCards == this.cards.length) {
              ...
              revealedValues = [];
              status.code = 4,
              status.message = 'Spiel vorbei! Attempts: ' + this.attempts +
                  ', Fehler: ' + this.mistakes;
            }
            else {
              status.code = 2,
              status.message = 'Partie.';
            }
          }
          cardSelection = [];
        }
          ...
          status.code = 1,
          status.message = 'Decke die Erste Karte auf.';
        }
      }
      else {
        status.code = 0,
        status.message = 'Karte ist bereits aufgedeckt.';
      }
